    ...
    <div className="overflow-x-auto rounded-xl border bg-white shadow-sm">
      <div className="min-w-[640px] p-4">
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className="w-72 border-b pb-2 pr-4 text-left font-medium text-slate-500">
                Course Learning Outcome
              </th>
              {standards.map((s) => (
                <th
                  key={s.code}
                  className="border-b pb-2 px-1 text-center font-medium text-slate-500"
                  title={s.title}
                >
          ...
          <tbody>
            {clos.map((clo, ri) => (
              <tr key={ri} className="hover:bg-slate-50">
                <td className="border-b py-2 pr-4 text-slate-700 align-middle">
                  {shortClo(clo)}
                </td>
                {standards.map((s) => {
                  const row = lookup.get(`${clo}|||${s.code}`);
                  return (
                    <td key={s.code} className="border-b py-2 px-1 text-center align-middle">
                      {row ? (
                        <HeatCell row={row} />
                      ) : (
